...
router.put(
  ...
  [
    ...
    body('contactAddress').optional({ checkFalsy: true }).trim().isLength({ max: 500 }),
    body('contactEmail').optional({ checkFalsy: true }).isEmail().normalizeEmail(),
    body('contactPhone').optional({ checkFalsy: true }).trim().isLength({ max: 64 }),
    body('mapEmbedUrl').optional({ checkFalsy: true }).trim().isLength({ max: 2000 }),
  ...
  async (req, res) => {
    try {
      ...
      const payload = {
        ...
        ...(req.body.contactAddress !== undefined && { contactAddress: req.body.contactAddress }),
        ...
        ...(req.body.mapEmbedUrl !== undefined && { mapEmbedUrl: req.body.mapEmbedUrl }),
