...
export default function PlatformAdminPage() {
  ...
  const toggleMaintenance = async () => {
    if (!token || maintenanceMode === null) return;
    setMaintenanceToggling(true);
    try {
      const next = !maintenanceMode;
      const current = await getSiteSettingsAdmin(token);
      await updateSiteSettingsAdmin(token, { ...current, maintenanceMode: next });
...
function SmsFeedbackPanel({ token }: { token: string | null }) {
  const [items, setItems] = useState<SmsFeedback[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  ...
  return (
    <div>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-lg font-bold text-slate-900">SMS Geri Bildirimleri</h2>
          <p className="mt-1 text-sm text-slate-600">
            İletişim sayfası ve &quot;SMS ile Geri Bildirim&quot; formundan gelen kayıtlar burada listelenir.
          </p>
        </div>
        <button
          type="button"
  ...
  return (
    <div>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-600">
          URL ve etiket girerek site genelinde kullanılabilecek linkler ekleyin. Görseli &quot;Yerelden görsel yükle&quot; ile Cloudinary’e yükleyerek canlı link (örn. https://res.cloudinary.com/.../image/upload/.../xxx.jpg) alabilir veya doğrudan görsel URL yapıştırabilirsiniz; linki görsele / görseli linke dönüştür butonları ile hızlı düzenleyebilirsiniz.
        </p>
      </div>
